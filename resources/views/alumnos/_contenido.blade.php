<div cl ass="card mb-3">
    <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <div class="card-header-action">
            <a href="{{route('alumnos.create')}}" class="btn btn-sm btn-primary ajax-sidepanel"><i class="fa-solid fa-plus"></i>&nbsp;{{$menuitems[$imenu1]['submenu'][$imenu2]['label']}}</a>
        </div>
    </div>
    ...
    <table id="tablelist" class="table table-striped table-bordered dataTable">
        <thead>
            ...
            <tbody>
                ...
                <tr class="">                                
                    <td class="" >
                        <a href="{{route('alumnos.edit',['alumno' => $alumno->id, 'sidepanel' =>true]) }}" title="Editar" class="btn btn-sm btn-primary ajax-sidepanel"><i class="fa-solid fa-pencil"></i></a> 
                        <!--<a href="{{route('alumnos.edit',$alumno->id)}}" title="Editar" class="btn btn-sm btn-primary"><i class="fa-solid fa-pencil"></i></a> -->
                        <button 
                        title="Eliminar registro"
                        type="button" 
                        class="btn btn-sm btn-danger"
                        onclick="openDeleteModalAlumno({{$alumno}})">
        ...
        <div class="modal fade" id="deleteModalAlumno" tabindex="-1" aria-labelledby="deleteModalLabelAlumno" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabelAlumno">Eliminar Alumno</h5>
                ...
                <div class="modal-footer">
                    <form action="{{route('alumnos.destroy',$alumno)}}" method="POST" class="inline" id="deleteFormAlumno">
                        @csrf
                        @method('DELETE')
        ...
        <!--FIN MODAL CONFIRMACIÓN-->            
            <script>
            function openDeleteModalAlumno(alumno) {                                                
                const modal = new bootstrap.Modal(document.getElementById('deleteModalAlumno'));
                const form = document.getElementById('deleteFormAlumno');                                                
                modal.show();
