<div class="card mb-3">
    <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <div class="card-header-action">
            <a href="{{route('aulas.create')}}"  class="btn btn-sm btn-primary ajax-sidepanel"><i class="fa-solid fa-plus"></i>&nbsp;{{$menuitems[$imenu1]['submenu'][$imenu2]['label']}}</a>
        </div>
    </div>
    ...
    <table id="tablelist" class="table table-striped table-bordered dataTable">
        <thead>
            ...
            <tbody>
                ...
                <tr class="">                                
                    <td class="" >                                                    
                        <a href="{{route('aulas.edit',['aula' => $aula->id, 'sidepanel' =>true]) }}" title="Editar" class="btn btn-sm btn-primary ajax-sidepanel"><i class="fa-solid fa-pencil"></i></a> 
                        <button 
                        title="Eliminar registro"
                        type="button" 
                        class="btn btn-sm btn-danger"
                        onclick="openDeleteModalAula({{$aula}})">
        ...
        <div class="modal fade" id="deleteModalAula" tabindex="-1" aria-labelledby="deleteModalLabelAula" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabelAula">Eliminar Aula</h5>
                ...
                <div class="modal-footer">
                    <form action="{{route('aulas.destroy',$aula)}}" method="POST" class="inline" id="deleteFormAula">
                        @csrf
                        @method('DELETE')
        ...
        <!--FIN MODAL CONFIRMACIÓN-->            
            <script>
            function openDeleteModalAula(aula) {                                                
                const modal = new bootstrap.Modal(document.getElementById('deleteModalAula'));
                const form = document.getElementById('deleteFormAula');                                                
                modal.show();
